    ...
    <>
          ...
          {paidDrugs.map((d) => (
            <div key={d.drug_key} style={{ marginTop: 10, paddingTop: 10, borderTop: "1px solid var(--border)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "baseline" }}>
                <span><b>{d.drug_key}</b> <span className="muted">{DRUG_META[d.drug_key]?.generic}</span></span>
                <span className="up" style={{ flexShrink: 0 }}>{money(num(d.pay_amount))}</span>
              </div>
      ...
      <div className="panel" style={{ padding: 18, marginBottom: 18 }}>
        ...
        {drugs.filter((d) => num(d.claims) > 0).map((d) => {
          ...
          return (
            <div key={d.drug_key} style={{ padding: "8px 0", borderTop: "1px solid var(--border)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 10, flexWrap: "wrap" }}>
                <span><b>{d.drug_key}</b>{paidForThis && <span title="paid for this drug"> 💵</span>}</span>
                {pct != null && (
                  paidForThis ? (
              ...
              <div className="muted" style={{ fontSize: 13, marginTop: 2 }}>
                {num(d.claims).toLocaleString()} claims · peer avg {Math.round(num(d.peer_unpaid_avg))}
              </div>
            </div>
          );
        })}
      ...
      {similar.length > 0 && primary && (
        <div className="panel" style={{ padding: 18 }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr 0.8fr 0.9fr", gap: 6, fontSize: 14, minWidth: 380 }}>
